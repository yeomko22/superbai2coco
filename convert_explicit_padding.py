...
def rescale_bbox(bbox, size):
    img_w, img_h = size
    x_c, y_c, w, h = box_cxcywh_to_xyxy(bbox)
    b = [int(x_c * img_w), int(y_c * img_h), int(w * img_w), int(h * img_h)]
    return b
...
def copy_files(src_paths: List[str], dst_paths: List[str]) -> None:
    ...
    for src_path, dst_path in tqdm(zip(src_paths, dst_paths), total=len(src_paths)):
        ...
        transformed_img = transform(src_img)
        transformed_img.save(dst_path)
...
def split(train_origin_dir: str, val_origin_dir, superbai_dir: str,
    ...
    src_train_paths = src_train_paths[:10]
    src_val_paths = src_val_paths[:10]
...
def get_coco_json(labels: List[str], categories: List[dict], image_info_dict: dict, bbox_dict: dict):
    ...
    for label in labels:
        ...
        for bbox_d in bboxes:
            bbox = bbox_d["bbox"]
            class_id = bbox_d["class_id"]
            bbox = rescale_bbox(bbox, (input_size, input_size))
